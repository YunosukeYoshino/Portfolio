...
export const Route = createFileRoute('/article/page/$page')({
  ...
  meta: ({ loaderData }) => {
    const pageNum = loaderData?.currentPage || 1
    const canonicalUrl = pageNum === 1 ? '/article' : `/article/page/${pageNum}`

    return [
      {
        title: pageNum === 1 ? 'Articles | Yunosuke Yoshino' : `Articles - Page ${pageNum} | Yunosuke Yoshino`,
      },
      {
        name: 'description',
...
function BlogListPage() {
  const { currentPage, totalPages } = Route.useLoaderData()
